        ...
        try {
          ...
          for (const section of sections) {
            ...
            if (sections.indexOf(section) > 0) {
              ...
              await delay(5000); // 5 second delay between sections
            }
              ...
              controller.enqueue(
                encoder.encode(
                  `data: ${JSON.stringify({ 
                    type: "content", 
                    section, 
                    content: `⚠️ Rate limit reached for ${section}. This is normal with Gemini's free tier. Please wait a moment and try again, or consider upgrading to a paid plan for higher limits.` 
                  })}\n\n`
                )
              )
              continue
...
function createSectionPrompt(section: string, inventionData: any, selectedOffice: any, format = "text"): string {
  ...
  const prompts = {
    Title: `Create a clear, concise patent title for this invention. The title should be descriptive but not overly long. ${formatInstruction} Based on: ${baseInfo}`,
    ...
    "Detailed Description": `Write a detailed description of the invention including how it works, its components, and implementation details. ${formatInstruction} Based on: ${baseInfo}`,
